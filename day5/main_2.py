...
def get_seeds(string: str):
    ...
    for i in range(len(seeds)):
        if i % 2 == 0:
            ...
            seeds_arr.append(range(n1, n1 + int(seeds[i])))
    print(seeds_arr)
    return seeds_arr
...
with (open('input.txt')) as f:
    ...
    for inp in inpr:
        ...
        if line_id.find('seeds') >= 0:
            input_arr['seeds'] = get_seeds(line_numbers)
        else:
            input_arr[line_id] = get_map(line_numbers)

    #print(input_arr['seeds'])
    min_loc = 999999999

    for ranges in input_arr['seeds']:
        ...
        for seed in ranges:
            ...
            #print('Seed ' + str(seed) + ', soil ' + str(soil) + ', fertiziler ' + str(fertilizer) + ', water ' + str(water) + ', light ' + str(light) + ', temperature ' + str(temperature) + ', humidity ' + str(humidity) + ', location ' + str(location))
